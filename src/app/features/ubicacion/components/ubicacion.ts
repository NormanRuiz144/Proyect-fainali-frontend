...
export class UbicacionComponent implements OnInit {
  ...
  municipiosParaFiltroSector = computed(() => {
    ...
    return this.municipios().filter(m => (m.id_departamento === depId || m.idDepartamento === depId));
  });
  ...
  abrirModalEditar(item: any) {
    ...
    const tab = this.activeTab();
    if (tab === 'departamentos') {
      this.formDepartamento.set({ id: item.id, nomDepartamento: item.nom_departamento || item.nomDepartamento });
    } else if (tab === 'municipios') {
      this.formMunicipio.set({ id: item.id, nomMunicipio: item.nom_municipio || item.nomMunicipio, idDepartamento: item.id_departamento || item.idDepartamento });
    } else if (tab === 'sectores') {
      const idMuni = item.id_municipios || item.idMunicipios || item.idMunicipio;
      this.formSector.set({ id: item.id, nomSector: item.nombre_sector || item.nombreSector || item.nomSector, idMunicipio: idMuni });
  ...
  guardar() {
    ...
    if (tab === 'departamentos') {
      ...
      if (!data.nomDepartamento) { alert('Nombre es requerido'); this.cargando.set(false); return; }
      
      if (modo === 'crear') {
        this.ubicacionService.crearDepartamento(data).subscribe({
          next: () => { this.cargarDatos(); this.cerrarModal(); },
          error: (err) => { alert(err.error?.mensaje || 'Error al crear'); this.cargando.set(false); }
        });
      } else if (data.id) {
        this.ubicacionService.actualizarDepartamento(data.id, data).subscribe({
          next: () => { this.cargarDatos(); this.cerrarModal(); },
          error: (err) => { alert(err.error?.mensaje || 'Error al actualizar'); this.cargando.set(false); }
        });
      }
    } else if (tab === 'municipios') {
      const data = this.formMunicipio();
      if (!data.nomMunicipio || !data.idDepartamento) { alert('Nombre y Departamento son requeridos'); this.cargando.set(false); return; }
      ...
      const data = this.formSector();
      if (!data.nomSector || !data.idMunicipio) { alert('Nombre y Municipio son requeridos'); this.cargando.set(false); return; }
